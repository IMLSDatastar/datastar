...
var manageMetadataWebpages = {	
	webpageClassSelector:'.webpage.metadata',	
    ...
    initPage: function() {
        ...
        if ($(this.webpageClassSelector).length) {  // make sure we have at least one webpage
    ...
    initWebpageData: function() {
        $(this.webpageClassSelector).each(function(index) {
            $(this).data(mwebpageData[index]);    
    ...
    bindEventListeners: function() {
        ...
        $('.metadata.remove').click(function() {
            manageMetadataWebpages.removeWebpage(this);
            return false;
        });
    ...
    initDragAndDrop: function() {
        ...
        var webpages = $('#metadataWebpageList');
        ...
        $('.webpageName.metadata').each(function() {
        ...
        webpages.sortable({
            ...
            update: function(event, ui) {
                manageMetadataWebpages.reorder(event, ui);
            }
        });     
    },
    ...
    reorder: function(event, ui) {
        var webpages = $('li.webpage.metadata').map(function(index, el) {
            return $(this).data('webpageUri');
        }).get();

        $.ajax({
            url: manageMetadataWebpages.reorderUrl,
            data: {
                predicate: manageMetadataWebpages.rankPredicate,
            ...
            success: function(data, status, request) {
                var pos;
                $('.webpage.metadata').each(function(index){
                    pos = index + 1;
                    ...
                    manageMetadataWebpages.setPosition(this, pos);                
                }); 
            },
            error: function(request, status, error) {
                ...
                if (ui) {
                    ...
                    var pos = manageMetadataWebpages.getPosition(ui.item),                       
                        nextpos = pos + 1, 
                        webpages = $('#metadataWebpageList'), 
                        next = manageMetadataWebpages.findWebpage('position', nextpos);
    ...
    findWebpage: function(key, value) {
        ...
        $('.webpage.metadata').each(function() {
            var webpage = $(this);
    ...
    removeWebpage: function(link) {
        ...
        $('a#showAddMetadataForm').addClass('disabledSubmit');
        $('a#showAddMetadataForm').attr('disabled', 'disabled');
        
        if ($(link)[0] === $('.remove.metadata:last')[0]) {
            removeLast = true;
        } 
        ...
        $.ajax({
            ...
            type: 'POST', 
            data: {
                deletion: $(link).parents('.webpage.metadata').data('webpageUri')
            },
            dataType: 'json',
            ...
            complete: function(request, status) {
                ...
                if (status === 'success') {
                    ...
                    webpage = $(this).parents('.webpage.metadata');
                    ...
                    webpage.fadeOut(400, function() {
                        ...
                        numWebpages = $('.webpage.metadata').length; // retrieve the new length after removing webpage from the DOM
                        
                        // If removed item not last, reorder to remove any gaps
                        if (numWebpages > 0 && ! removeLast) {                            
                            manageMetadataWebpages.reorder();
                        }
                            
                        // If fewer than two webpages remaining, disable drag-drop
                        if (numWebpages < 2) {
                            manageMetadataWebpages.disableDD();
                        }                           
                        ...
                        $('a#showAddMetadataForm').removeClass('disabledSubmit');
                        $('a#showAddMetadataForm').attr('disabled', '');
                    });
    ...
    disableDD: function() {
        ...
        $('#metadataWebpageList').sortable({ disable: true } )
        ...
        $('.webpageName.metadata').removeAttr('title');
    }

};

$(document).ready(function() {   
    manageMetadataWebpages.onLoad();
}); 
